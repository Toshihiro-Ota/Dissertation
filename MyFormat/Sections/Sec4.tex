...
%\newcommand{\fundbox}{\tikz{\node[fnode, semithick, minimum size=8pt] {}}}  %7pt
...
(1) We first apply to the brane tilings the general prescription to construct
...
(2) For such a case, we will obtain a dictionary between supersymmetric gauge theory and
integrable lattice model, which is called \emph{Gauge/YBE correspondence}.
(3) The brane tiling construction can be mapped into the setups of the so-called class-$\CS$ theories.
We proceed to the introduction of surface defects in the simplest case of the theories of class-$\CS$.
(4) Then we find that the surface defects act on the supersymmetric index by
...
\begin{figure}
  \centering
    ...
    \end{tikzpicture}
    \caption{Structure of section \ref{sec:surface}. (1) \& (2) Along the line of the general prescription
...
%(see figure \ref{fig:D5NS5config})
.
...
If the theory is described by a quiver diagram, translation between gauge
...
The unitarity relations in lattice model side are satisfied
...
To write down the index, let $a_{i}=e^{2\pi \iu u_{i}}$ be the fugacity
for the flavor group $\U(1)_i$ associated with the $i$th zig-zag path.
Also, we use the Pochhammar symble $(z;q)_{\infty}=\prod_{k=0}^{\infty}\left(1-q^{k}z\right)$.
...
The index $\mathcal{I}_{\mathcal{T}}$ of a four-dimensional $\mathcal{N}=1$ theory
...
\cite{Seiberg:1994bz, Spiridonov:2014cxa}. The theory on the left-hand side
...
it by this factor.
%$\Gamma\big(\prod_{i}a_{i}^{NF_{i}};p,q\big)$, which cancels
%the contribution from the corresponding baryon.
...
\begin{figure}
  ...
  \qquad\qquad
    ...
    (b) A spin system associated with the quiver diagram (a), defined on the same lattice.
    Spins and their interaction specify a lattice model.
    The nearest-neighbor interaction may be chiral.}
    \label{fig:gauge_ybe}
  \end{figure}


In the expression of the full index \eqref{eq:fullindex},
...
Spectral parameter & Holonomy of $\U(1)$ gauge field  \tabularnewline  % R-charge
...
As a preparation, here we review some basic facts of class-$\CS$ theory
...
\centering
  ...
  In the quiver, the two nodes in the same column are identified and
  it reproduces the quiver in figure \ref{fig:linear_quiver}.
...
of class-$\mathcal{S}$,
%\cite{Gaiotto:2009we,Gaiotto:2009hg},
...
which we denote by $R^{\diamondsuit}$.
This R-matrix
...
$R^{\diamondsuit}$ of Bazhanov-Sergeev type,
...
Based on these observations, a natural counterpart of the L-operator for the
diamond quiver
\begin{equation}
    L^{\diamondsuit}(c,(a,b))
    =\
...
%The supersymmetric index with surface defect has been studied in
...
$w$. Using formula \eqref{eq:gamma_residue}, we find that the contribution from
...
According to the general argument, the surface defect creates a dashed line
