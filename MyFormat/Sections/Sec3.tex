...
\begin{figure}
  \centering
    ...
    (1) A transfer matrix consisting of trigonometric L-operators is constructed in a particular form.
...
The vevs of such line operators in $\CN=2$
supersymmetric gauge theories have been exactly computed via
...
In~\cite{Ito:2011ea}, the localization calculation for the vevs of
half-BPS Wilson-'t Hooft lines on $S^1 \times_\eps \R^2 \times \R$ in
the Coulomb phase was performed.
...
The origin of the AGT correspondence is six-dimensional $\CN = (2,0)$ supersymmetric field theory,
...
On one hand, let us compactify $\R^4_{\eps_1,\eps_2}$ to an ellipsoid $S^4_{\bs}$,
...
This brane configuration is nothing but the one shown in section \ref{sec:defects}.
...
%Compactify $\R_9 \to S^1_9$, reduce on $S^1_3$, and apply T-duality $S^1_9\to \check{S}^1_9$,
%then we now have type IIB string theory setup
...
Note that though $E$ is a punctured torus, this is not $C_{1,n}$ for
...
\begin{figure}
  \centering
    \begin{tikzpicture}[baseline=(x.base)]  \node (x) at (0,0) {\vphantom{x}};
      ...
      \begin{scope}[scale=0.7, font=\small, xshift=4cm]
        ...
        \node[black!70] (p1) at (-2,-0.6) {$\bullet$};
        \draw[ultra thick, dotted] (1.8,-0.7) arc (-65:-42:1.8);
        \node[red!70] at (0.4,-0.6) {\scriptsize $\times$};  %$\bullet$
      \end{scope}
  ...
  given by the coordinate of the lines on the Riemann surface.}
\label{fig:top_hol}
\end{figure}
...
\end{equation*}
and the equivalence is achieved by string dualities.
